		...
		if (CurrentPlayer) {
			if (CurrentPlayer->HasAuthority()) {
				if (CurrentPlayer->GetIntractionActorCanNextActive()) {     // 상호 작용 가능하면
                    ...
                    {
                        CurrentPlayer->ServerMontagePlayWithSession(CurrentPlayer->InteractionMontage, CurrentPlayer->InteractionSectionName[7], 1.f);     // 윗 부분 치기
                        return;
                    }
                    ...
                    {
                        CurrentPlayer->ServerMontagePlayWithSession(CurrentPlayer->InteractionMontage, CurrentPlayer->InteractionSectionName[6], 1.f);     // 중간 부분 치기
                        return;
                    }


					CurrentPlayer->ServerMontagePlayWithSession(CurrentPlayer->InteractionMontage, CurrentPlayer->InteractionSectionName[5], 1.f);         // 하단 부분 치기
				}
				else {
